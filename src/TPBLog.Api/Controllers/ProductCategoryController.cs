...
namespace TPBlog.Api.Controllers
{
    ...
    public class ProductCategoryController : ControllerBase
    {
        ...
        public async Task<IActionResult> CreateProductCategory([FromBody] CreateUpdateProductCategoryRequest request)
        {
            ...
            var post = _mapper.Map<CreateUpdateProductCategoryRequest, IC_ProductCategory>(request);
            _unitOfWork.IC_ProCategories.Add(post);
        ...
        {
            var post = await _unitOfWork.IC_ProCategories.GetByIdAsync(id);
            if (post == null)
            {
                return NotFound();
            }
        ...
        public async Task<IActionResult> DeleteProductCategory([FromQuery] Guid[] ids)
        {
            foreach (var id in ids)
            {
                ...
                if (await _unitOfWork.IC_ProCategories.HasProduct(id))
                {
                    return BadRequest("Danh mục đang chứa sản phẩm, không thể xóa");
                }
                _unitOfWork.IC_ProCategories.Remove(post);
            }
            var result = await _unitOfWork.CompleteAsync();
            return result > 0 ? Ok() : BadRequest();
        }
        ...
        {
            var category = await _unitOfWork.IC_ProCategories.GetByIdAsync(id);
            if (category == null)
            {
                return NotFound();
            }
        ...
        {
            var result = await _unitOfWork.IC_ProCategories.GetPagingProductCategoryAsync(keyword, pageIndex, pageSize);
            return Ok(result);
        }
        ...
        {
            var query = await _unitOfWork.IC_ProCategories.GetAllAsync();
